...
public class ArraySet<T> extends AbstractSet<T> {
	...
	private void expandCapacity()
	{
       ...
       T[] larger = (T[]) (new Object[(contents.length * 2)]);
		...
		// Don't forget to update the contents of this set using the new array.
		contents = larger;
	}
	...
	public SetADT<T> intersection(SetADT<T> anotherSet)
	{
		 ...
		 */
ArraySet<T> intersectedSet = new ArraySet<T>();
...
return intersectedSet;
	 ...
	 */
    public SetADT<T> difference(SetADT<T> anotherSet)
    {
    	ArraySet<T> differenceSet = new ArraySet<T>(contents.length);
	...
	public SetADT<T> union (SetADT<T> set)
	{
		 ...
		 */
         ArraySet<T> unionedSet = new ArraySet<T>(this);
	...
	{
		SetADT<Integer> s1 = new ArraySet<Integer>(6);
		for(int i=0; i<6; i++)
			s1.add(i);
		SetADT<Integer> s2 = new ArraySet<Integer>(6);
		for(int i=2; i<8; i++)
			s2.add(i);
