...
public class Lottery {
	...
	// Number of numbered balls to be picked, usually 6.  
	private int maxPick;
	...
	public void sellTicket()
	{
		 ...
		 */
       tickets.add(new LotteryTicket(++ numOfTicketsSold, this));
	}
	 ...
	 */
	public void sellTicket(SetADT<Integer> numbers)
	{
		/* !!!! Create an appropriate LotteryTicket object and update
		 * 		the set of tickets sold so far.
		 */
		tickets.add(new LotteryTicket( ++numOfTicketsSold, this, numbers));
	}
	...
	public int[] drawNumbers()
	{
		...
		{
			int chosenNumber = availableNumbers.removeRandom();
			results[i] = chosenNumber;
			winningNumbers.add(chosenNumber);
		}
	...
	public String hasWon()
	{
		 ...
		 *      How to check if someone has won?
		 *      	We can perform intersection on two sets 
		 ...
		 *      Return a message regarding the result. 
		 */
		String message = "";
		for(LotteryTicket t : tickets)
		{
		SetADT<Integer>	purchasedNumbers = t.chosenNumbers();
		if(purchasedNumbers.intersection(winningNumbers).size() == maxPick){
			message += t.toString();
		}
