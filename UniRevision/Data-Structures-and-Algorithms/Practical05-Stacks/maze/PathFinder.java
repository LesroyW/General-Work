...
public class PathFinder {
		...
		while (!found && !reversedPath.isEmpty()) {
          ...
          if(currentPosition.equals(maze.exit()))
          {
        	  ...
        	  return reversedPath;
				 ...
				 */
                List<Position> possibleMoves = maze.potentialMovesFrom(currentPosition);
                
                if(possibleMoves.size() == 0)
                {
                	reversedPath.pop();
                } else {
                	reversedPath.push(possibleMoves.get(0));
                	maze.markVisited(possibleMoves.get(0));
                }
	...
	private StackADT<Position> reverse() {
		 ...
		 */
         LinkedStack<Position> reversedStack = new LinkedStack<>();
         while(!reversedPath.isEmpty())
         {
        	
        	 reversedStack.push(reversedPath.pop());
         }
