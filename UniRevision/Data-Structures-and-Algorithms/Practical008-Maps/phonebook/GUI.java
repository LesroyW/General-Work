...
public class GUI {
	 ...
	 */
	private PhoneBook<String,Correspondence,String> phoneBook;
	...
	public GUI(PhoneBook<String, Correspondence, String> phoneBook) {
		 ...
		 */ 
		ArrayList<String> fontNames = GUIUtil.getSuitableFontNames('ö');
		if(fontNames.size() > 0) {
			font = new Font(fontNames.get(0),Font.BOLD,20);
		}
		else {
			 /* Throw an exception when no suitable font can be used 
			  * to display the requested character (set).
			  */
			throw new UnsupportedCharsetException("Character set with the character ö.");
		}
