...
public class CircularArrayQueue<T> implements QueueADT<T>
{
	...
	public T dequeue() throws IllegalStateException
	{
		 ...
		 */
            T elementToDequeue = contents[front];
            contents[front] = null;
	...
	public boolean isEmpty()
	{
		 ...
		 * 		any element.
		 */  
      return contents[front] == null && front == rear;
	}
	...
	public int size()
	{
		...
		if(isEmpty()) { // The queue is empty.
			...
			if( rear == front && contents[front] != null) { // The queue is full.
				return contents.length;
			}
			else
			{   // The queue isn't full nor empty.
				/* ++++ rear is the next available slot in the array. */ 
				if(front < rear) {
					size = rear - front;
				}
				else {
					size = contents.length - front + rear;
				}
			}
		}
	...
	public String toString()
	{
		 ...
		 */
		int FrontOfQueue = front;
		for(int i = 0; i < size(); i++)
		{
			result += contents[i].toString() + '\n';
			FrontOfQueue = (FrontOfQueue + 1) % contents.length;
		}
